...
// ✅ Import helper functions instead of BASE_URL
...
const GalleryManagement = () => {
  ...
  // ✅ Transform API data to album structure with full image URLs
  const transformApiDataToAlbums = (apiData) => {
    const albumsMap = {};
    ...
    apiData.forEach((item) => {
      ...
      if (item.filepath) {
        const mediaItem = {
          ...
          url: getFullImageUrl(item.filepath),
          // ✅ Keep original relative path for any future updates
  ...
  // ✅ Handle file upload - store relative path in API
  const handleFileUpload = async (files, albumId) => {
    const fileList = Array.from(files);
    if (fileList.length === 0) return;
    ...
    try {
      for (const file of fileList) {
        try {
          ...
          // ✅ Ensure we have a proper relative path for API storage
          ...
          // ✅ Create gallery item in database with RELATIVE path
          const apiData = {
            title: file.name,
            type: file.type.startsWith("image/") ? "photo" : "video",
            filepath: relativePath, // ✅ Store relative path in API
            category: album.name,
            isactive: true,
          };
      ...
      // Reload gallery to get updated data (will transform to full URLs)
      await loadGallery();
      setError("");
    } catch (err) {
      setError("Failed to upload files");
  ...
  // ✅ Get cover image URL - already has full URL from transform
  const getCoverImageUrl = (album) => {
    if (!album.coverImage) return null;
    const coverMedia = album.media.find(
      (media) => media.id === album.coverImage
  ...
  // ✅ Update selectedAlbum when albums change (to reflect new uploads)
  useEffect(() => {
    if (selectedAlbum) {
      ...
      <div className="mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-10">
            {albums.map((album) => (
              <AlbumCard
                ...
                onUpload={(files) => handleFileUpload(files, album.id)}
                onDelete={() => showDeleteConfirmation(album.id)}
              />
            ))}
          </div>
        )}
        ...
        {selectedAlbum && (
          <AlbumDetailView
            ...
            onUpload={(files) => handleFileUpload(files, selectedAlbum.id)}
            onBack={() => setSelectedAlbum(null)}
            onDeleteMedia={(mediaId) => showDeleteConfirmation(selectedAlbum.id, mediaId)}
            onSetCover={(mediaId) => setCoverImage(selectedAlbum.id, mediaId)}
...
// ✅ Separate Album Card Component
const AlbumCard = ({ album, getCoverImageUrl, onSelect, onUpload, onDelete }) => {
...
// ✅ Separate Album Detail View Component
      ...
      <div className="p-6 border-b border-gray-200">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              {album.media.length} media items • Created{" "}
              {new Date(album.createdAt).toLocaleDateString()}
            </p>
          </div>
          <div className="flex gap-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {album.media.map((media) => (
              <MediaItem
                key={media.id}
                media={media}
                isCover={album.coverImage === media.id}
                onDelete={() => onDeleteMedia(media.id)}
                onSetCover={() => onSetCover(media.id)}
                formatFileSize={formatFileSize}
              />
            ))}
          </div>
...
// ✅ Separate Media Item Component
const MediaItem = ({ media, isCover, onDelete, onSetCover, formatFileSize }) => {
  return (
    <div className="group relative bg-gray-100 rounded-lg overflow-hidden hover:shadow-lg transition-all duration-200">
      {/* ✅ Media URL already has full URL from transform */}
      {media.type === "image" || media.type === "photo" ? (
        <img
          src={media.url}
          alt={media.name}
