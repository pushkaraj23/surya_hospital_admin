...
//     // Transform album data to API structure
//     const transformAlbumToApiData = (album, mediaItem = null) => {
//         if (mediaItem) {
//             return {
...
//                 const newItem = await addGalleryItem(apiData);
...
//                         'http://localhost:8654/api/uploads/single', 
//                         formData,
//                         {
//                             headers: {
...
//                         coverImage: album.coverImage || newMedia[0]?.id
//                     }
//                     : album
//             );
...
//         if (window.confirm('Are you sure you want to delete this album? All media will be lost.')) {
//             try {
//                 const album = albums.find(a => a.id === albumId);
//                 if (!album) return;
...
//             const updatedMedia = album.media.filter(media => media.id !== mediaId);
//             let newCoverImage = album.coverImage;

//             if (album.coverImage === mediaId) {
//                 newCoverImage = updatedMedia.length > 0 ? updatedMedia[0].id : null;
//             }

//             const updatedAlbums = albums.map(album =>
//                 album.id === albumId
//                     ? { ...album, media: updatedMedia, coverImage: newCoverImage }
//                     : album
//             );
...
//         const coverMedia = album.media.find(media => media.id === album.coverImage);
...
//                     <div>
//                         <h1 className="text-2xl font-bold text-gray-900">  <PhotoLibrary /> Gallery Management</h1>
...
//                             <h2 className="text-2xl font-bold text-gray-900 mb-4"> <Add fontSize="large" /> Create New Album</h2>
//                             <input
//                                 type="text"
...
//                         {albums.map(album => (
//                             <div key={album.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-200">
//                                 <div
...
//                                                 <span className="text-sm font-medium">{album.media.length} items</span>
...
//                                 <div className="p-4">
//                                     <h3 className="font-semibold text-gray-900 mb-3 truncate">{album.name}</h3>
...
//                                                 multiple
//                                                 accept="image/*,video/*"
//                                                 onChange={(e) => handleFileUpload(e.target.files, album.id)}
//                                                 className="hidden"
//                                             />
//                                         </label>
//                                         <button
...
//                                         {selectedAlbum.media.length} media items • Created {new Date(selectedAlbum.createdAt).toLocaleDateString()}
//                                     </p>
//                                 </div>
//                                 <button
...
//                                     multiple
//                                     accept="image/*,video/*"
//                                     onChange={(e) => handleFileUpload(e.target.files, selectedAlbum.id)}
//                                     className="hidden"
//                                     id="file-upload"
//                                 />
...
//                                     </h3>
//                                     <p className="text-gray-600 mb-1">Upload multiple photos or videos</p>
//                                     <p className="text-sm text-gray-500">Supports JPEG, PNG, GIF, MP4, MOV</p>
//                                 </label>
//                             </div>
//                         </div>
...
//                                 <h4 className="font-semibold text-gray-900 mb-4">Uploading Files</h4>
//                                 <div className="space-y-3">
//                                     {uploadingFiles.map((fileName, index) => (
//                                         <div key={index} className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200">
//                                             <span className="text-sm font-medium text-gray-700 truncate flex-1 mr-4">{fileName}</span>
...
//                                         <div key={media.id} className="group relative bg-gray-100 rounded-lg overflow-hidden hover:shadow-lg transition-all duration-200">
//                                             {media.type === 'image' ? (
//                                                 <img
//                                                     src={media.url}
//                                                     alt={media.name}
...
//                                                 />
//                                             ) : (
//                                                 <video className="w-full aspect-square object-cover">
...
//                                                         className={`px-3 py-2 rounded-lg text-xs font-medium transition-colors duration-200 ${selectedAlbum.coverImage === media.id
...
//                                                 <p className="text-xs font-medium truncate mb-1">{media.name}</p>
...
//                             Create your first album to start organizing your photos and videos into collections like Health Camps or Hospital Events.
//                         </p>
//                         <button
...
//                                     {getCoverImageUrl(album) ? (
//                                         <img
//                                             src={`http://localhost:8654/${getCoverImageUrl(album)}`}
//                                             alt={album.name}
//                                             className="w-full h-full object-cover"
//                                         />
//                                     ) : (
...
//                                                 <img
//                                                     src={`http://localhost:8654/${media.url}`}
//                                                     alt={media.name}
//                                                     className="w-full aspect-square object-cover"
//                                                 />
//                                             ) : (
//                                                 <video className="w-full aspect-square object-cover" controls>
//                                                     <source src={`http://localhost:8654/${media.url}`} />
//                                                 </video>
//                                             )}
                                        ...
                                        <div key={media.id} className="group relative bg-gray-100 rounded-lg overflow-hidden hover:shadow-lg transition-all duration-200">
                                            ...
                                            <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-60 transition-all duration-200 flex items-center justify-center opacity-0 group-hover:opacity-100">
                                                <div className="flex gap-2">
                                                    ...
                                                    <button
                                                        onClick={() => showDeleteConfirmation(selectedAlbum.id, media.id)}
                                                        className="bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded-lg text-xs font-medium transition-colors duration-200"
                                                    >
                                                        Delete
                                                    </button>
