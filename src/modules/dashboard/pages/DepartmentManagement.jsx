...
export default function DepartmentManagement() {
  ...
  // Add custom styles for Quill editor and description content rendering
  useEffect(() => {
    const style = document.createElement('style');
    style.id = 'quill-description-styles';
    style.textContent = `
      /* Quill Editor Styles */
    ...
    const existingStyle = document.getElementById('quill-description-styles');
    if (existingStyle) {
    ...
    return () => {
      const styleToRemove = document.getElementById('quill-description-styles');
  ...
  const handleSubmit = () => {
    ...
    const payload = {
      name: formData.name.trim(),
      description: formData.description, // Rich text HTML
      bannerimage: formData.bannerimage.trim(),
      isactive: formData.isactive,
      services: parseServices(formData.services),
      success_rate: formData.success_rate ? parseInt(formData.success_rate) : null,
  ...
  return (
    <div className="py-2 min-h-screen bg-gray-50">
      <div className="mx-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  accept="image/*"
                  onChange={handleFileUpload}
                  className="w-full border border-gray-300 rounded-lg px-4 py-3 bg-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100 cursor-pointer"
                />
                {formData.bannerimage && (
              ...
              <div className="md:col-span-2">
                ...
                <div className="quill-editor-wrapper">
                  <ReactQuill 
                    ...
                    theme="snow"
                    placeholder="Write a detailed description about the department..."
                  />
                </div>
                <p className="text-xs text-gray-500 mt-2">Use the toolbar to format text with bold, italic, lists, headings, and more.</p>
              </div>
                ...
                {Array.isArray(departments) && departments.length > 0 ? (
                  departments.map((dept) => (
                    <tr key={dept.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4">
                        {hasValidDescription(dept.description) ? (
                          <div 
                            className="text-sm text-gray-600 max-w-xs description-content line-clamp-2"
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-2 space-y-6">
                    ...
                    <div>
                      ...
                      {hasValidDescription(selectedDepartment.description) ? (
                        <div 
                          className="text-gray-600 bg-gray-50 rounded-lg p-4 description-content"
                          dangerouslySetInnerHTML={{ __html: selectedDepartment.description }}
                        />
                      ) : (
                        <p className="text-gray-400 bg-gray-50 rounded-lg p-4">No description available.</p>
                      )}
                    </div>
        ...
        {/* Mutation Loading States */}
        {(createMutation.isLoading || updateMutation.isLoading || deleteMutation.isLoading || toggleStatusMutation.isLoading) && (
          <div className="fixed bottom-4 right-4 bg-amber-600 text-white px-4 py-2 rounded-lg shadow-lg flex items-center space-x-2 z-50">
            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
            <span className="text-sm">
              {createMutation.isLoading && "Creating department..."}
              {updateMutation.isLoading && "Updating department..."}
