...
const ContactDetailsManager = () => {
  ...
  const loadContactDetails = async () => {
    ...
    try {
      ...
      setContactDetailsList(contactsArray);
      if (contactsArray.length > 0) {
        selectContact(contactsArray[0]);
      }
    } catch (error) {
      console.error("Error loading contact details:", error);
      alert("Error loading contact details: " + error.message);
  ...
  const handleUpdate = async () => {
    ...
    try {
      await updateContactDetails(activeContactId, formData);
      alert("Contact details updated successfully!");
      loadContactDetails();
    } catch (error) {
      alert("Error updating contact details: " + error.message);
  ...
  const currentContact = contactDetailsList.find((c) => c.id === activeContactId);
