...
const CoreValuesManager = () => {
  ...
  // ✅ imagePreview stores FULL URL for display
  const [imagePreview, setImagePreview] = useState("");
  ...
  const handleImageUpload = async (file) => {
    ...
    try {
      // 1. Upload image
      const responsePath = await uploadCoreValueImage(file);

      // 2. Convert to Relative Path
      const relativePath = getRelativePath(responsePath);

      // 3. Update form data with RELATIVE path (for API)
      setFormData((prev) => ({
        ...prev,
        image: relativePath,
      }));

      // 4. Update preview with FULL URL (for Display)
      setImagePreview(getFullImageUrl(relativePath));
  ...
  // Copy image URL to clipboard (Copies Full URL)
  const copyImageUrl = (relativeUrl) => {
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    try {
      // formData.image is already relative, safe to post
      await createCoreValue(formData);
      alert("Core value created successfully!");
      resetForm();
      setMode("list");
  ...
  const handleUpdate = async () => {
    ...
    setLoading(true);
    try {
      // formData.image is already relative, safe to put
      await updateCoreValue(currentValue.id, formData);
      alert("Core value updated successfully!");
      resetForm();
      setMode("list");
  ...
  const handleView = async (id) => {
    ...
    try {
      ...
      const relativeImg = getRelativePath(value.image);
      ...
      // For viewing, we can just use getFullImageUrl in the render, 
      // but let's set preview just in case we switch to edit mode from view
      setImagePreview(relativeImg ? getFullImageUrl(relativeImg) : "");
              ...
              <div>
                ...
                >
                  ...
                  {imagePreview ? (
                    <div className="relative group">
                      <img
                        // ✅ Use imagePreview (Full URL) for display
                        src={imagePreview}
                        alt="Core Value"
                        className="w-full h-64 object-cover rounded-lg"
                        onError={(e) => {
              ...
              <div>
                ...
                <div className="border rounded-lg overflow-hidden">
                  {formData.image ? (
                    <div className="relative">
                      <img
                        // ✅ Use getFullImageUrl for View Mode
