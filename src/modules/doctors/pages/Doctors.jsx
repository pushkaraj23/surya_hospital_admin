...
const DoctorsComponent = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    experience_years: 0,
    departmentid: "",
    photo: "",           // ✅ Relative path for saving
    photoPreview: "",    // ✅ Full URL for preview
    bio: "",
    schedule: {},
    isactive: true,
    isexpert: false,
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [docsRaw, deps] = await Promise.all([
        getDoctors(),
        fetchDepartments(),
      ]);

      // ✅ Transform doctors to have full image URLs for display
      const docs = docsRaw.map((doctor) => ({
        ...doctor,
        photo: getFullImageUrl(doctor.photo),
        // Keep original relative path for editing
        originalPhotoPath: doctor.photo,
      }));

      // ✅ Transform departments to have full image URLs
      const depsWithImages = deps.map((dept) => ({
        ...dept,
  ...
  const handleEdit = async (doctor) => {
    ...
    setFormData({
      ...
      experience_years: fullDoc.experience_years,
      departmentid: fullDoc.departmentid || "",
      photo: fullDoc.photo || "",  // ✅ Original relative path for saving
      photoPreview: getFullImageUrl(fullDoc.photo),  // ✅ Full URL for preview
      bio: fullDoc.bio || "",
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        photo: relativePath,                      // ✅ Relative for saving
        photoPreview: getFullImageUrl(relativePath), // ✅ Full URL for preview
      }));
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    const relativePhotoPath = getRelativePath(formData.photo);
    ...
    const submissionData = {
      ...
      photo: relativePhotoPath,  // ✅ Send relative path to API
      bio: formData.bio,
      schedule: scheduleForSubmission,
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
      ...
      <div className="p-4 border-b border-gray-100">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            {/* ✅ Photo already has full URL from loadData transform */}
            {doctor.photo ? (
              <img
                src={doctor.photo}
                alt={doctor.fullname}
...
// ✅ Separate Doctor Form Modal Component
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto scrollbarHide">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                className="w-full border border-gray-300 rounded-lg px-4 py-3 bg-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
              />
              {/* ✅ Show preview using photoPreview (full URL) or fallback */}
              {(formData.photoPreview || formData.photo) && (
                <div className="mt-3 flex items-center gap-3">
                  <img
                    src={formData.photoPreview || getFullImageUrl(formData.photo)}
                    alt="Preview"
                    className="w-16 h-16 rounded-full object-cover border-2 border-blue-200"
                  />
                  <button
          ...
          <div className="md:col-span-2">
            ...
            <p className="text-xs text-gray-500 mt-2">
              Set availability times for each day (24-hour format). Leave blank
              for days off.
            </p>
          </div>
